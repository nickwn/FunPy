...
import { getUtils, isHtmlEscape, removeHtmlEscape, TableElement, setInner } from 'vs/editor/contrib/rtv/browser/RTVUtils';
...
function containsLEAPFragmentDelim(str: string) {
	return str.match("# ---") !== null;
}
...
class LEAPFocusController {
	...
	public resetDecorations() {
		...
		for (let k = 1; k <= maxline; k++) {
			if (containsLEAPFragmentDelim(ctrl.getLineContent(k))) {
				if (start === -1) {
					start = k;
				} else {
					end = k;
		...
		if (start !== -1 && end !== -1) {
			ctrl.setVisibilityRange(start, end);
			...
			this._decoration1 = this._controller.addDecoration(range1, { description: 'LEAP Fragment Focus', inlineClassName: 'rtv-code-fade', zIndex: -10000000000 });
			this._decoration2 = this._controller.addDecoration(range2, { description: 'LEAP Fragment Focus', inlineClassName: 'rtv-code-fade', zIndex: -10000000000 });

		} else {
			ctrl.changeViewMode(ctrl.viewMode);
		}
...
export class RTVController implements IRTVController {
	...
	private _LEAPFocusController: LEAPFocusController | null = null;
	...
	get LEAPFocusController(): LEAPFocusController | null {
		return this._LEAPFocusController;
	}

	set LEAPFocusController(lc: LEAPFocusController | null) {
		this._LEAPFocusController?.destroyDecorations();
		this._LEAPFocusController = lc;
	}
	...
	private async onDidChangeModelContent(e: IModelContentChangedEvent) {
		...
		if (this.LEAPFocusController !== null) {
			this.LEAPFocusController.resetDecorations();
		}
	}
	...
	private onKeyDown(e: IKeyboardEvent) {
		if (e.keyCode === KeyCode.Escape) {
			...
			if (this.LEAPFocusController === null) {
				this.LEAPFocusController = new LEAPFocusController(this);
			} else {
				this.changeViewMode(this.viewMode);
				this.LEAPFocusController = null;
			}
		}
